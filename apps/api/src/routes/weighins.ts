...
weighinsRoutes.post('/', async (c) => {
  ...
  if (body.subjectType === 'user') {
    if (body.subjectId !== userId) throw new AppError('FORBIDDEN', 403, 'Cannot log another user\'s weight');
  } else {
    const pet = await Pet.findById(body.subjectId);
    if (!pet || String(pet.userId) !== userId) throw new AppError('NOT_FOUND', 404, 'Pet not found');
  }
  ...
  if (body.subjectType === 'user') {
    ...
    user.targets = {
      ...
      lastAdjustedAt: adj.applied ? new Date().toISOString() : targets.lastAdjustedAt ?? user.targets?.lastAdjustedAt ?? null,
      adjustmentLog: log,
    };
    ...
    const pet = await Pet.findById(body.subjectId);
    if (!pet) throw new AppError('NOT_FOUND', 404, 'Pet not found');
    ...
    pet.targets = {
      ...targetFields,
      adaptivePct: pct,
      lastAdjustedAt: adj.applied ? new Date().toISOString() : pet.targets?.lastAdjustedAt ?? null,
      adjustmentLog: log,
    };
...
weighinsRoutes.get('/', async (c) => {
  ...
  if (q.subjectType === 'user' && q.subjectId !== userId) {
    throw new AppError('FORBIDDEN', 403, 'Cannot read another user\'s weigh-ins');
  }
