...
petsRoutes.post('/:id/feedings', async (c) => {
  ...
  const grams = body.grams ?? Math.round(portion / meals);
  if (!(grams > 0)) throw new AppError('VALIDATION_ERROR', 400, 'grams must be > 0');
  ...
  return c.json({ feeding: FeedingSchema.parse(callToApi(feeding)), today }, 201);
});
